  ...
  <section id="content" class="flex">
    ...
    <div id="menu-content" class="flex flex-col w-full pb-[30px]">
      <div class="nav flex justify-between p-5 border-b border-[#EEEEEE]">
        <h1 class="font-extrabold text-[24px]">Data Pasien</h1>
        <a href="{{ route('pasien.create') }}" class="bg-[#6436F1] text-white font-bold px-5 py-2 rounded-full hover:shadow-lg transition">Tambah Pasien</a>
      </div>
          ...
          <tbody class="text-sm text-[#4B5563]">
            @foreach ($pasien as $item)
              <tr class="border-t border-[#EEEEEE] hover:bg-[#F3F4F6]">
                ...
                <td class="p-3">{{ \Carbon\Carbon::parse($item->tanggal_lahir)->format('d M Y') }}</td>
                <td class="p-3 capitalize">{{ $item->jenis_kelamin }}</td>
                <td class="p-3 flex gap-2">
                  <a href="{{ route('pasien.edit', $item->id) }}" class="text-blue-600 hover:underline">Edit</a>
                  <form action="{{ route('pasien.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')">
                    @csrf
                    @method('DELETE')
